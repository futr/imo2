...
//---------------------------------------------------------------------------
void __fastcall TSatViewMainForm::DrawImg( TImage *screen, TImage *back_screen, bool drawModeExp, int zoompos, int img_cent_x, int img_cent_y, ColorMap *cmap )
{
	/* 描画関数 */
...
void __fastcall TSatViewMainForm::DrawImg( void )
{
    ...
    SatImage->Picture->Bitmap->Width = sc_w;
    SatImage->Picture->Bitmap->Height = sc_h;

    SatImage->Picture->Bitmap->PixelFormat = pf24bit;
    SatImage->Picture->Bitmap->HandleType = bmDIB;

    /* 処理中 */
    str = "描画中";
    SatImage->Canvas->Font->Size   = 12;
    SatImage->Canvas->Font->Color  = clRed;
    SatImage->Canvas->Brush->Color = clWhite;
    SatImage->Canvas->TextOutA( sc_w / 2 - SatImage->Canvas->TextWidth( str ) / 2, sc_h / 2 - SatImage->Canvas->TextHeight( str ) - 10, str );
    Application->ProcessMessages();
    ...
    /* 画像の中心座標 */
    img_cent_x = ScrImgHor->Position;
    img_cent_y = ScrImgVert->Position;
    ...
    /* キャンパスクリア */
    SatImage->Canvas->Brush->Color = clWhite;
    SatImage->Canvas->FillRect( Rect( 0, 0, sc_w, sc_h ) );
    ...
    if ( mag >= 1 ) {
        ...
        for ( i = 0; i < img_read_yc; i++ ) {
            ...
            for ( k = 0; k < img_read_xc; k++ ) {
                ...
                if ( b_draw_mode_exp ) {
                    ...
                    red   = GetUCharValue( color_map->getR() );
                    green = GetUCharValue( color_map->getG() );
                    blue  = GetUCharValue( color_map->getB() );
                }
            ...
            for ( j = 0; j < skip; j++ ) {
                /* 行ポインタ取得 */
                rgb = (RGBTRIPLE *)SatImage->Picture->Bitmap->ScanLine[j + draw_y + i * skip];
        ...
        for ( i = 0; i < img_read_yc; i++ ) {
            ...
            /* 行ポインタ取得 */
            rgb = (RGBTRIPLE *)SatImage->Picture->Bitmap->ScanLine[draw_y + i];
